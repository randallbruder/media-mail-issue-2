public class Thermal implements SerialPortEventListener {
  ...
  private void init() {
    write(new byte[] { 
      27, 55, 7, (byte) ((_heatTime << 24) >> 24), (byte) ((_heatInterval << 24) >> 24)
    }
    );
  ...
  public void printBarcode(String s, int type) {
    ...
    write(new byte[] { 
      29, 107, (byte) ((type << 24) >> 24)
    }
    );
    for (int i = 0; i < s.length(); i++) {
      write(new byte[] { 
        (byte) ((s.charAt(i) << 24) >> 24)
      }
      );
    }
  }
  ...
  public void printBitmap(int width, int height, byte[] bmp) {
    ...
    for (int x = 0; x < recursion; x++) {
      ...
      if (width == 384) {
        ...
        write(new byte[] { 
          18, 118, (byte)calculatedHeight, (byte)calculatedWidth
        }
        );
        write(bmpLine);
      }
        ...
        write(new byte[] { 
          18, 42, (byte) ((newHeight << 24) >> 24), (byte) (((width / 8) << 24) >> 24)
        }
        );
        write(bmpLine);
      }
  ...
  public void setBarcodeHeight(int barcodeHeight) {
    write(new byte[] { 
      27, 45, (byte) ((barcodeHeight << 24) >> 24)
    }
    );
  }
